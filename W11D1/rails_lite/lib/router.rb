...
class Router
  ...
  # for syntactic sugar :)
  def draw(&proc) # get Regexp.new("^/cats$"), Cats2Controller, :index
    self.instance_eval(&proc) #expand the scope of the router instance vars to the proc
  end

  # make each of these methods that
  # when called add route

  # get(get(pattern, controller_class, action_name), #post(pattern, controller_class, action_name
