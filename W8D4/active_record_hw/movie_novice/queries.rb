...
def top_titles
  ...
  Movie.where('movies.score >= 9').select('movies.title, movies.id')
end
...
def pulp_fiction_actors
  ...
  Actor
    .joins(:castings).where('actors.id = castings.actor_id')
    .joins(:movies).where('movies.id = castings.movie_id')
    .select(:id,:name)
