...
def just_like_orange
  ...
  execute(<<-SQL)
    ...
    FROM
      cats
    JOIN
      cats AS orange ON orange.breed = cats.breed
    WHERE
      orange.name = 'Orange' AND cats.name != 'Orange'
    ORDER BY
      cats.name 
  SQL
end
...
def toys_that_jet_owns
  ...
  FROM cats
    ...
    JOIN cattoys AS cat2 ON toys.id = cat2.toy_id
    JOIN cats AS other_cats ON cat2.cat_id = other_cats.id
